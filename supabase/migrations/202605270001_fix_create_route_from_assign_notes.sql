-- Fix create_route_from_assign_notes for mixed reference/test data.
-- Route statuses are global reference data in production, so they must not
-- depend exclusively on p_is_test when the app is running in development.
