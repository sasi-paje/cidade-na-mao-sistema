-- Fix create_route_from_assign_notes:
--   1. Use id_previous_route_invoice (not id_previous_route_invoice_id)
--   2. Insert history only when event_type column exists (no else branch)
...
begin
  ...
  limit 1;
  if not found then raise exception 'Status inicial da rota nao encontrado'; end if;
  ...
  order by case when is_test = p_is_test then 0 else 1 end
  limit 1;
  if not found then raise exception 'Status inicial da entrega nao encontrado'; end if;

  select coalesce(max(route_code::integer), 0) + 1 into v_route_code
  from public.trx_route
  where is_test = p_is_test and route_code ~ '^[0-9]+$';
  v_route_code := lpad(v_route_code::text, 6, '0');

  select data_type = 'ARRAY' into v_assistant_is_array
  from information_schema.columns
  where table_schema = 'public' and table_name = 'trx_route' and column_name = 'assistant';
  ...
  if coalesce(v_assistant_is_array, false) then
    ...
    insert into public.trx_route (
      ...
      v_route_code, p_departure_date, v_id_route_status, v_id_route_delivery_status,
      p_id_vehicle, p_id_driver, p_area, array_to_string(p_assistant, ', '), p_is_test, true, p_id_route_responsible
    ) returning id into v_route_id;
  end if;
  ...
  if p_invoice_ids is not null and array_length(p_invoice_ids, 1) > 0 then
    foreach v_invoice_id in array p_invoice_ids loop
      ...
      insert into public.rel_route_invoice (
        id_route, id_fiscal_invoice, assigned_at, is_test, is_active,
        attempt_number, id_previous_route_invoice, planned_box_quantity, planned_amount
      ) values (
        v_route_id, v_invoice_id, now(), p_is_test, true,
  ...
  -- Insere histórico somente se a tabela tiver a coluna event_type
  select exists (
  ...
  if v_has_history_event_type then
    insert into public.trx_route_history (
      ...
      v_route_id, 'ROUTE_CREATED', 'Rota Criada',
